...
// @author          nabby (includes code written by ballparts, Ben, despair & probably some others)
// ==/UserScript==
...
var options = {
    ...
    'ShowDisplayedName':         { display:'"Display Name" (off = Reserved Name)',          type:'checkbox',      value:true,                                 title:''},
    'ShowDWMStatsTable':         { display:'D/W/M Stats',                                   type:'checkbox',      value:true,                                 title:''},
    'ShowDWMPUPTable':           { display:'D/W/M PUP\'s',                                  type:'checkbox',      value:true,                                 title:''},
    'Show300StatsTable':         { display:'Rolling-300 Stats',                             type:'checkbox',      value:true,                                 title:''},
    'Show300PUPTable':           { display:'Rolling-300 PUP\'s',                            type:'checkbox',      value:true,                                 title:''},
    'ShowPUPsPerGame':           { display:'"Per Game" Stats',                              type:'checkbox',      value:false,                                title:''},
    'AddCurrentServerButton':    { display:'Show "Updating Stats / Server" button',         type:'checkbox',      value:true,                                 title:''},
    'ServerStatsOnPlayButton':   { display:'Also show stats on "Play Now" button',          type:'checkbox',      value:false,                                title:''},
...
$(document).ready(function() {
    console.log("START: YAIS");

if ((PageLoc == 'server') || (PageLoc == 'joining')) {
    var LoggedIn = setProfilePage();
    
    $('body').append('<div id="YAIS_Menu_Button" style="position:fixed; top:10px; left:10px; background:#fff; color:#0b0; opacity:0.2; padding:12px 18px; font-size:18px; text-align:center; border-radius:6px; cursor:pointer">YAIS</div>');
    $('#YAIS_Menu_Button').after('<div id="YAIS_Menu" style="position:fixed; top:10px; left:10px; background:#444; color:#0b0; width:300px; padding:5px 5px 20px 5px; font-size:12px; border-radius:6px; overflow:hidden; z-index:100; display:none"><div style="font-size:18px; text-align:center; text-shadow:1px 2px 1px #777; text-decoration:underline;">YAIS Options</div></div>');

    var YAIS_Selections = options; //defaults
    if (GM_getValue('YAIS_Selections')) { //overwrite the defaults with the user selections...
        var saveSelections_Flag = false;
        $.each(GM_getValue('YAIS_Selections'), function(key, value) {
            if (YAIS_Selections[key]) {
                if (YAIS_Selections[key].type != 'manual') YAIS_Selections[key].value = value.value;
            } else { //saved key no longer exists in our object (probably changed/removed on an update)
                saveSelections_Flag = true;
            }
        });
        if (saveSelections_Flag) GM_setValue('YAIS_Selections', YAIS_Selections);
    }

    if ($('#WLT').length) {
        $('#WLT').after('<div id="YAIS" style="text-align:center; margin:20px auto 0 auto; padding:10px; width:-webkit-fit-content; border-radius:8px; box-shadow:#fff 0px 0px 10px; background:rgba(0,0,0,0.1); white-space:nowrap;"></div>');
    } else {
        $('a[href^="/maps"]').parent('div').after('<div id="YAIS" style="text-align:center; margin:20px auto 0 auto; padding:10px; width:-webkit-fit-content; border-radius:8px; box-shadow:#fff 0px 0px 10px; background:rgba(0,0,0,0.1); white-space:nowrap;"></div>');
    }


    //=================================================================
    //Adds a new button for the server you're on with updating stats...
    if ( (YAIS_Selections.AddCurrentServerButton.value) && (PageLoc != 'profile') ) {
        ...
        function createButton() {
            ...
            $('#e_AddCurrentServerButton').append('<a id="currentServer" href="http://'+serverURL+'" class="button" title="Click to reload this page...">'+capitaliseFirstLetter(serverName)+'<span class="strong">'+serverLocation+'<br></span><span class="stats"></div>');
    ...
    if (LoggedIn) {
        ...
        getProfilePage.done(function(data) {
            ...
            $(document).addStyle('#flairtable tr { color:#ffffff; background:#13415B; cursor:pointer }');
            $(document).addStyle('#flairtable th { color:#13415B; background:#ffffff; cursor:default }');
            $(document).addStyle('#flairtable tr:hover { background:#1C5F86 }');
            $(document).addStyle('#flairtable td { padding:1px 5px }');
        ...
        if (PageLoc == 'server') {
            $(document).addStyle('.SettingsMenu_Button{display:inline-block; width:70px; height:20px; line-height:20px; margin:0 10px; padding:2px 10px; background:#777; color:#fff; font-size:15px; border:1px solid #fff; border-radius:6px;}');
            $(document).addStyle('.SettingsMenu_Button:hover{background:#0b0; box-shadow:#fff 0 0 5px; }');
                ...
                if (tagpro.serverHost.length && settings.length) {
                    ...
                    postUpdateSettings.done(function(data) {
                        if ($.parseJSON(data).success === true) {
                            ...
                            console.log( "Profile Settings Data NOT Saved!");
                        }
                    });
    ...
    $(document).addStyle('table.board td, table.board th { padding:2px 5px }');
    $(document).addStyle('.leaderboard_button { display:inline-block; margin:0 10px; padding:3px 10px; font-size:20px; background:#000; border:1px solid; border-radius:8px; width:65px; cursor:pointer }');
    $('#YAIS').append('<div id="e_ShowLeaderboardPage" style="position:absolute; margin:0 auto; left:0; right:0; top:150px; padding:20px; font-size:10px; text-shadow:2px 1px 2px #000; height:60%; width:500px; background:#666; overflow-y:auto; z-index:6000; display:none"></div>');
    ...
    if (LoggedIn) {
        getProfilePage.then(function(data) {
            ...
            if (YAIS_Selections.PriddyButtons.value) {
                $(document).addStyle('.YAIS:hover { border-bottom:1px solid #0f0; }');
                $('div.buttons').next('div').children('a').addClass('YAIS');
                $(document).addStyle('a.button { color:'+YAIS_Selections.BigButtonColorText.value+'; background:'+YAIS_Selections.BigButtonColor.value+'; border-radius:15px }');
                $(document).addStyle('a.button:hover { background:'+YAIS_Selections.BigButtonColorHover.value+'; box-shadow:#ffffff 0 0 8px }');
    ...
    $(document).addStyle('#YAIS_Menu_Mask{background:#000000 url(\'http://i.imgur.com/WKZPcQA.gif\') no-repeat center 300px; position:absolute; opacity:0.7; height:100%; width:100%; top:0; left:0; z-index:5000; display:none;}');
    ...
    $('#YAIS_Menu_Button').mouseleave(function() {
        $('#YAIS_Menu_Button').fadeTo(200, 0.2);
    });
