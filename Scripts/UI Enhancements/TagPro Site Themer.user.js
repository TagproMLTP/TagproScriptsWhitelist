...
var presets = {
    'default':                    { display:'Default',           values:{ TST_Buttons:'#ACE600',         TST_ButtonsText:'#000000',   TST_ButtonsBorder:'#608100',   TST_ButtonsHover:'#ACE600',         TST_Links:'#00FF00', TST_BackgroundColor:'#000000',           TST_ShowButtonsBorder:3,  TST_ShowButtonsShadow:0,  TST_ButtonsBorderRadius:0,  TST_ButtonsFont:'Monospace',      TST_MainLogo:'/images/logo.png'} },
    'uired':                      { display:'UI Red',            values:{ TST_Buttons:'#DD0000,#4A0000', TST_ButtonsText:'#FFFFFF',   TST_ButtonsBorder:'#EE0000',   TST_ButtonsHover:'#EE0000',         TST_Links:'#999999', TST_BackgroundColor:'#5E1313,#000000',   TST_ShowButtonsBorder:1,  TST_ShowButtonsShadow:8,  TST_ButtonsBorderRadius:5,  TST_ButtonsFont:'Courier New',    TST_MainLogo:'http://i.imgur.com/O4uUPLD.png'} },
    'uiblue':                     { display:'UI Blue',           values:{ TST_Buttons:'#0000AA',         TST_ButtonsText:'#FFFFFF',   TST_ButtonsBorder:'#FFFFFF',   TST_ButtonsHover:'#400080,#400080', TST_Links:'#999999', TST_BackgroundColor:'#070837',           TST_ShowButtonsBorder:8,  TST_ShowButtonsShadow:0,  TST_ButtonsBorderRadius:25, TST_ButtonsFont:'Arial Black',    TST_MainLogo:'http://i.imgur.com/enMGRMN.png'} },
    'custom1':                    { display:'Custom #1',         values:{ TST_Buttons:'#008040,#00AAAA', TST_ButtonsText:'#000000',   TST_ButtonsBorder:'#AAAA00',   TST_ButtonsHover:'#80FFFF',         TST_Links:'#8080FF', TST_BackgroundColor:'#000040,#004000',   TST_ShowButtonsBorder:2,  TST_ShowButtonsShadow:8,  TST_ButtonsBorderRadius:25, TST_ButtonsFont:'Calibri',        TST_MainLogo:'https://cdn2.colorlib.com/wp/wp-content/uploads/sites/2/2014/02/Olympic-logo.png'} },
    'custom2':                    { display:'Custom #2',         values:{ TST_Buttons:'#8080C0,#8000FF', TST_ButtonsText:'#FFFFFF',   TST_ButtonsBorder:'#FFFFFF',   TST_ButtonsHover:'#400040',         TST_Links:'#FFFFFF', TST_BackgroundColor:'#242424,#B1B1B1',   TST_ShowButtonsBorder:0,  TST_ShowButtonsShadow:8,  TST_ButtonsBorderRadius:0,  TST_ButtonsFont:'Arial Narrow',   TST_MainLogo:'https://www.google.co.nz/images/srpr/logo11w.png'} },
    'custom3':                    { display:'Custom #3',         values:{ TST_Buttons:'#122345,#C0C0C0', TST_ButtonsText:'#000000',   TST_ButtonsBorder:'#808000',   TST_ButtonsHover:'#000040,#C0C0C0', TST_Links:'#8080C0', TST_BackgroundColor:'#000000,#552B00',   TST_ShowButtonsBorder:2,  TST_ShowButtonsShadow:30, TST_ButtonsBorderRadius:8,  TST_ButtonsFont:'Georgia',        TST_MainLogo:'http://screenrant.com/wp-content/uploads/Star-Wars-Logo-Art.jpg'} },
};
...
tagpro.ready(function() {
    ...
    if (GM_getValue('TST_Selections')) { //overwrite the defaults with the user selections...
        saveSelections_Flag = false;
        $.each(GM_getValue('TST_Selections'), function(key, value) {
            if (TST_Selections[key]) {
                TST_Selections[key].value = value.value;
                TST_Selections[key].enabled = value.enabled;
            } else { //saved key no longer exists in our object (probably changed/removed on an update)
                saveSelections_Flag = true;
            }
        });
        if (saveSelections_Flag) GM_setValue('TST_Selections', TST_Selections);
    } else {
        GM_setValue('TST_Selections', TST_Selections);
    }

    var TST_Presets = jQuery.extend(true, {}, presets);
    if (GM_getValue('TST_Presets')) { //overwrite the defaults with the user selections...
        saveSelections_Flag = false;
        $.each(GM_getValue('TST_Presets'), function(key, value) {
            if (TST_Presets[key]) {
                TST_Presets[key].values = value.values;
                TST_Presets[key].display = value.display;
            } else { //saved key no longer exists in our object (probably changed/removed on an update)
                saveSelections_Flag = true;
            }
        });
        if (saveSelections_Flag) GM_setValue('TST_Presets', TST_Presets);
    } else {
    ...
    //=================================================================
    if (PageLoc !== 'ingame') {
        $('body').append('<div id="TST_Menu_Button" style="position:fixed; bottom:20px; right:10px; background:#fff; color:#0b0; opacity:0.10; padding:12px 18px; font-size:18px; text-align:center; border-radius:25px; cursor:pointer">Site Themer</div>');
        $('#TST_Menu_Button').after('<div id="TST_Menu" style="position:fixed; bottom:0px; right:0px; background:#111; color:#C0B866; padding:30px 10px 40px 10px; font-size:12px; border:2px solid #AAA; border-radius:6px; overflow:hidden; z-index:1000; display:none"></div>');
        ...
        $.each(TST_Selections, function(key, value) {
            if (value.type === 'color') {
                ...
                $.each(fonts, function(index, fontName) {
                    $('#'+key).append('<option value="'+fontName+'"' + (fontName === TST_Selections.TST_ButtonsFont.value ? ' selected' : '') + '>'+fontName+'</option>');
                });
        ...
        $('#TST_Menu_Container').after('<div style="position:absolute; bottom:15px; width:100%">' +
                                       ...
                                       '<a href="javascript:void(0);" id="TST_Export" style="margin:2px 0 0 10px; padding:2px; font-size:11px; color:#FFF; background:#666666; border-radius:4px; cursor:pointer" title="Export Current Settings to File">Export</a>' +
                                       '<a href="javascript:void(0);" id="TST_Import" style="margin:2px 0 0 2px; padding:2px; font-size:11px; color:#FFF; background:#666666; border-radius:4px; cursor:pointer" title="Import Current Settings to File">Import</a><input type="file" id="TST_Import_Open" accept="text/json" style="display:none">' +
        ...
        $('#TST_Reset').on('click', function() {
            var response = confirm("Are you sure you want to reset all values?\n(your saved themes will also be lost)");
            if (response) {
                GM_deleteValue('TST_Selections');
                GM_deleteValue('TST_Presets');
                response = confirm("All values have been reset - changes will take effect after page reload.\n\nDo you want reload this page now?");
                if (response) {
                    window.location.reload();
                } else {
                    showMessage('Page reload needed to see changes...', 5);
                }
            }
        });
        ...
        $('#TST_ButtonsFont').on('change', function() {
        ...
        $('#TST_Menu_Button').mouseleave(function() {
            $('#TST_Menu_Button').fadeTo(200, 0.10);
        });
    ...
    function groupCSS(){
        ...
        if ((tagpro.group.socket) && (tagpro.group.socket.connected)) {
            $('html').css('background', TST_Selections.TST_BackgroundColor.value.split(',')[0] || '#000000');
