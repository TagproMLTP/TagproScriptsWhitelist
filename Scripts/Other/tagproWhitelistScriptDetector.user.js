...
(function() {
    ...
    unsafeWindow.Function = (function(Function) {
        function _Function() {
            ...
            var userScriptMatchPattern = /\("\w+?:.+?'(.*?)'[^]+?[\r\n]{2}([^]+?)\}\)\}[\r\n]{0,4}$/;
            if (typeof userScript === 'string' && userScriptMatchPattern.test(userScript)) {
                var userScriptMatches = userScript.match(userScriptMatchPattern);
        ...
        function fetchAllScripts(scriptUrls) {
            scriptUrls.forEach(function(url) {
                getContent(url, function(script) {
                    whitelistedScripts.push(formatUserscript(script));
