...
var format = {
    ...
    minutes: function(x) {return x + ' minutes';},
    caps: function(x) {return x == 0 ? 'No Limit' : x;},
    percentage: function(x) {return x * 100 + '%';},
    milliseconds: function(x) {return x / 1000 + ' seconds';},
    potato: function(x) {return x == 0 ? 'Disabled' : x / 1000 + ' seconds';},
    boolean: function(x) {return x ? 'Yes' : 'No';}
};
...
function updateSettings() {
    ...
    for(var key in defaults) {
        ...
        if(val != setting.default) {
            $('<li>')
              .attr('setting', key)
              .text(setting.label + (key === 'buffDelay' ? '' : ': '))// + setting.format(val)))
...
tagpro.group.socket._callbacks.chat[0] = function(e) {
    ...
    msg.append(e.message).appendTo('#chat');
...
tagpro.group.socket.on('member', function(e) {
    if(e.leader) {
        if(leader !== e.id && leader !== "")
            $('<div>').text("'" + e.name + "' was set as group leader.").appendTo('#chat');
        leader = e.id;
    }
});
...
mapIds = { // positive is unfortunate-maps.jukejuice.com, negative is maps.jukejuice.com. always prefer unfortunate-maps because it has better previews. this was a lot of work so respect it pls
    ...
    //Rugby MISSING
    'Rush': 12192,
    //SaM MISSING //sharks and minnows
    'Shine': 4917,
    'Simplicity': 377,
    'Smirk': 4913,
